...
# License: MIT
# https://github.com/adelerhof/proxmox/raw/main/LICENSE
# bash -c "$(wget -qLO - https://github.com/adelerhof/proxmox/raw/main/misc/cron-update-lxcs.sh)"
...
add() {
  ...
  sh -c '(crontab -l -u root 2>/dev/null; echo "0 0 * * 0 PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin /bin/bash -c \"\$(wget -qLO - https://github.com/adelerhof/proxmox/raw/main/misc/update-lxcs-cron.sh)\" >>/var/log/update-lxcs-cron.log 2>/dev/null") | crontab -u root -'
  clear
  echo -e "\n To view Cron Update LXCs logs: cat /var/log/update-lxcs-cron.log"
}

remove() {
  (crontab -l | grep -v "github.com/adelerhof/proxmox/raw/main/misc/update-lxcs-cron.sh") | crontab -
